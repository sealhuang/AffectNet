...
def fire_module(x, fire_id, squeeze, expand):
    ...
    x      = Conv2D(squeeze, (1, 1), padding='same', kernel_initializer='glorot_uniform',bias_initializer='zeros',kernel_regularizer = regularizers.l2(regularizer_value),data_format='channels_last', activation= 'relu',name=s_id + 'sq1x1')(x)
    x      = BatchNormalization()(x)

    left   = Conv2D(expand, (1, 1), padding='same', kernel_initializer='glorot_uniform',bias_initializer='zeros',kernel_regularizer = regularizers.l2(regularizer_value),data_format='channels_last', activation= 'relu',name=s_id + 'exp1x1')(x)
    left   = Dropout(0.7)(left)

    right  = Conv2D(expand, (3, 3), padding='same', kernel_initializer='glorot_uniform',bias_initializer='zeros',kernel_regularizer = regularizers.l2(regularizer_value),data_format='channels_last', activation= 'relu',name=s_id + 'exp3x3')(x)
    right  = Dropout(0.7)(right)

    y      = concatenate([left, right], axis=3, name=s_id + 'concat')
    return y
...
x  = Conv2D(128, (5, 5), padding='same', kernel_initializer='glorot_uniform',bias_initializer='zeros',kernel_regularizer = regularizers.l2(regularizer_value),data_format='channels_last', activation= 'relu',name= 'conv12')(x)
...
# checkpoint
outputFolder = './output-model-scratch-sqnet01'
